...
| `operator.affinity` | object | `{"nodeAffinity":{"preferredDuringSchedulingIgnoredDuringExecution":[{"weight":1,"preference":{"matchExpressions":[{"key":"node-role.kubernetes.io/master","operator":"In","values":[""]}]}},{"weight":1,"preference":{"matchExpressions":[{"key":"node-role.kubernetes.io/control-plane","operator":"In","values":[""]}]}}]}}` | Operator's affinity configuration |
...
| `operator.metricsExporter.seLinuxType` | string | `spc_t` | SELinux domain label (aka process type) to apply to the `metrics-exporter` container (leave empty for the containers engine to apply the default one, i.e. `container_t`) |
